...
export class AgendaService {
  ...
  public getAgendamento(): Observable<any> { 
    
    let matricula = localStorage.getItem('idBeneficiario');
    let nascimento = localStorage.getItem('nascimentoFormatado');

    return this.http.get('https://apis.seconci-sp.org.br:3011/consultaClientes/agenda/' + matricula + '/' + nascimento, httpOptions );
  }

  public getExames(matricula: string, recurso: string, data: string, horario: string) {
    return this.http.get('https://apis.seconci-sp.org.br:3011/consultaAgenda/exames/' + matricula + "/" + recurso + "/" + data + "/" + horario , httpOptions );
  }
  

  public getPreparo(codigoPreparo) {
    return this.http.get('https://apis.seconci-sp.org.br:3011/consultaAgenda/preparo/codigo/' + codigoPreparo, httpOptions );
  }
