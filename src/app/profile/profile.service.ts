import { Injectable } from '@angular/core';
import { HttpClient, HttpHeaders } from '@angular/common/http';
import { Observable } from 'rxjs';


const httpOptions = {
 
  headers: new HttpHeaders({
    'Content-Type':  'application/json',
    'id': 'auth0_5a4c332eca00bd289d0b27e5',
    'token': 'eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiIsImtpZCI6IlJVUTNNell6TTBReU1qVkJORGRETnpZd1FURkRSVVZDT1VReU1qSTFPRE0yUTBWQlFUTkJOQSJ9.eyJuaWNrbmFtZSI6Im5hZGFsIiwibmFtZSI6IkVkc29uIFNhbnRhbmEgQW5kcmFkZSAiLCJwaWN0dXJlIjoiaHR0cHM6Ly9zLmdyYXZhdGFyLmNvbS9hdmF0YXIvZThkZmMzOWY5ZTFkOWQ3NmY4NDhhMGVmM2VmNjNjYzQ_.D6djHMMRZOGjOx5zbf-CWYRZIBvCF0OE8vuLeqGIxvEpkANOf3bOhUDcdf4i2w8JRKy8qA_66LjiE-0v11mLVeYeHeHFZwBj9WvlDcFRY58gChwoJi4JYMFk3oZ-Qe6dkJ-tWVsUqs2brOEjN0mSuwqPLSlxisdKunIPhJhdXuxJP-Uqlseh2jw7GK0XY1WtDAXdnoqRAjf70H60q2BdYqz8t6JNr_hCpUbJDc1ppwijEnq0l7EYfj1KjTP_MePNwh4VZN-uoFao8M10EQ8Pd8BQgD3XSOFZpUBh45UnoLdL24NfZDUqqheB5D-93GnsHsLwD0IGmTiXr3t-NZQFWw'
  })

};

@Injectable({
  providedIn: 'root'
})
export class ProfileService {

  constructor(
    private http: HttpClient
  ) {}

  public getClienteProfile(idBeneficiario): Observable<any> {
    return this.http.get("https://apis.seconci-sp.org.br:3011/consultaClientes/paciente/" + idBeneficiario, httpOptions );
  }

  public getCEP(cep: string): Observable<any> {
    return this.http.get("https://apis.seconci-sp.org.br:3011/consultaClientes/cep/" + cep, httpOptions );
  }

  public updateProfile(profile) {
    return this.http.put("https://apis.seconci-sp.org.br:3011/consultaClientes/cadastro/", JSON.stringify(profile), httpOptions);
  }

}

